...
RUN set -x \
  ...
  ; py_url=$(curl -sSL https://api.github.com/repos/indygreg/python-build-standalone/releases/tags/${py_build} \
...
RUN set -eux \
  ...
  ; curl -sSL ${yq_url} | tar zxf - ./yq_linux_amd64 && mv yq_linux_amd64 $UTILS_ROOT/yq \
  \
  ...
  ; curl -sSL ${xh_url} | tar zxf - -C ${UTILS_ROOT} --strip-components=1 --wildcards '*/xh' \
